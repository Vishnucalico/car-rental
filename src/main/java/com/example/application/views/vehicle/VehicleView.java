...
@PageTitle("Vehicle")
@Route(value = "Vehicle/:carID?/:action?(edit)", layout = MainLayout.class)
@AnonymousAllowed
public class VehicleView extends Div implements BeforeEnterObserver {

    private final String CAR_ID = "carID";
    private final String CAR_EDIT_ROUTE_TEMPLATE = "Vehicle/%s/edit";
    ...
    @Autowired
    public VehicleView(CarService carService) {
        this.carService = carService;
        addClassNames("vehicle-view");
        ...
        grid.asSingleSelect().addValueChangeListener(event -> {
            if (event.getValue() != null) {
                ...
                clearForm();
                UI.getCurrent().navigate(VehicleView.class);
            }
        });
    ...
    public void beforeEnter(BeforeEnterEvent event) {
        ...
        if (carId.isPresent()) {
            ...
            if (carFromBackend.isPresent()) {
                ...
                // refresh grid
                refreshGrid();
                event.forwardTo(VehicleView.class);
            }
        }
    }
